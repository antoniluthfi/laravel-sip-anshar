...
<head>
    ...
    <title>Laporan Pengiriman Pesanan</title>
</head>
<body>
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th class="display-6"><strong>Ekspedisi</strong></th>
                <th class="display-6"><strong>Ongkir</strong></th>
                <th class="display-6"><strong>Tanggal Pengiriman</strong></th>
        ...
        <tbody>
            @forelse ($data as $key => $item)
                <tr>
                    @php
                        $tgl_pengiriman = strtotime($item->tanggal_pengiriman);
                        $tgl_pengiriman = date("d M Y", $tgl_pengiriman);
                    @endphp

                    <td class="display-6">{{ $key + 1 }}</td>
                    <td class="display-6">{{ $item->kode_pengiriman }}</td>
                    <td class="display-6">{{ $item->kode_pesanan }}</td>
                    <td class="display-6">{{ ucwords($item->user->name) }}</td>
                    <td class="display-6">{{ $item->user->nomorhp }}</td>
                    <td class="display-6">{{ $item->ekspedisi->nama_ekspedisi }}</td>
                    <td class="display-6">{{ number_format($item->ongkir) }}</td>
                    <td class="display-6">{{ $tgl_pengiriman }}</td>
            ...
            @empty
                <tr>
                    <td colspan="{{ $dari !== $sampai ? '9' : '8' }}" class="text-center">Tidak ada data</td>
                </tr>
            @endforelse
        </tbody>
    </table>
