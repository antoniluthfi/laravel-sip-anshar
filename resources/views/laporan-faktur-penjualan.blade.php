...
$total_harga = 0;
foreach($data as $val) {
    $total_faktur += (int) $val->nominal;
    $total_terhutang += (int) $val->terhutang;
    $total_harga += (int) $val->pesananPenjualan->total_harga;
}
...
<body>
    ...
    <h3 class="display-5">Laporan Faktur Penjualan</h3>
    <div class="container mb-4">
        <div class="row">
            <div class="float-left" style="width: 30%;">  
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">Tanggal</p>
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">Marketing</p>
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">Total Faktur</p>
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">Total Terhutang</p>
            ...
            <div class="float-right" style="width: 69%;">
                ...
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">: Rp. {{ number_format($total_faktur) }}</p>
                <p class="lead-2 ml-1 mt-0 mb-0 p-0">: Rp. {{ number_format($total_terhutang) }}</p>
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th class="display-6"><strong>No Faktur</strong></th>
                <th class="display-6"><strong>Kode Pesanan</strong></th>
                <th class="display-6"><strong>Pelanggan</strong></th>
                <th class="display-6"><strong>Tgl Faktur</strong></th>
                <th class="display-6"><strong>Total Faktur</strong></th>
                <th class="display-6"><strong>Terhutang</strong></th>
                <th class="display-6"><strong>Total Biaya</strong></th>
                <th class="display-6"><strong>Status</strong></th>
        ...
        <tbody>
            @forelse ($data as $key => $item)
                <tr>
                    ...
                    <td class="display-6">{{ $item->no_faktur }}</td>
                    <td class="display-6">{{ $item->kode_pesanan }}</td>
                    <td class="display-6">{{ ucwords($item->user->name) }}</td>
                    <td class="display-6">{{ date_format($item->created_at, "d M Y") }}</td>
                    <td class="display-6 text-right">{{ number_format($item->nominal) }}</td>
                    <td class="display-6 text-right">{{ number_format($item->terhutang) }}</td>
                    <td class="display-6 text-right">{{ number_format($item->pesananPenjualan->total_harga) }}</td>
                    <td class="display-6">{{ $item->terhutang ? 'Belum Lunas' : 'Lunas' }}</td>
            ...
            @empty
                <tr>
                    <td colspan="{{ $dari !== $sampai ? '10' : '9' }}" class="display-6 text-center">Tidak ada data</td>
                </tr>
            @endforelse 
        </tbody>
    </table>
